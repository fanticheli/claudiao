...
const STRONG = ['Dev@2024!prod', 'Testando#2024', 'Mudar$2024xyz', 'MyPassword2024abc', 'Edutalent_Localiza9', 'A1a-Zk8Pq2LmN0aBcDxy', 'q7$Lk^2[mZ|x<9Tr]pW3>vB8'];
...
describe('standards-credentials: secrets that must be blocked', () => {
  [
    `PGPASSWORD='${STRONG[5]}' psql -h lovelace-prod.x.rds.amazonaws.com -U app`,
    `psql 'postgresql://v-oidc-igo-lovelace-abc:${STRONG[5]}@lovelace-replica-01-prod.x.rds.amazonaws.com:5432/db'`,
    ...
    `psql postgresql://v-oidc-igor:${STRONG[5]}@localhost:5433/lovelace-prod`,
    ...
    `cat > login.json <<'EOF'\n{"email":"someone@digai.ai","password":"Igor@2024x"}\nEOF`,
    `PGPASSWORD=lovelaceProd2024 psql -h replica`,
  ].forEach(detects);

  test('vault CLI table and JSON passwords pasted in a prompt are detected', () => {
    assert.ok(findCredentials(`Key                Value\npassword           ${STRONG[5]}\nusername           v-oidc-igo-x`).length > 0);
...
describe('standards-credentials: daily commands that must pass', () => {
  [
    ...
    '~/.claude/scripts/db-query lovelace-prod "select 1"',
    ...
    'DATABASE_URL=postgresql://lovelace:lovelace@localhost:5432/lovelace npx prisma migrate dev',
    ...
    'DB_SECRET_ARN=arn:aws:secretsmanager:us-east-1:123456789012:secret:lovelace-db-AbCdEf npm start',
    ...
    'aws secretsmanager get-secret-value --secret-id lovelace/prod --profile prod',
...
describe('standards-credentials: hook decisions', () => {
  ...
  test('Bash with credential is denied pointing to db-query', () => {
    ...
    assert.match(output.hookSpecificOutput.permissionDecisionReason, /db-query/);
  });
